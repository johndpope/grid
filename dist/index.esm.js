...
import React__default, { memo as memo$1, useEffect, useRef, useState, useCallback, createContext, isValidElement, cloneElement, useLayoutEffect, useMemo, useContext, createElement as createElement$1, useReducer, forwardRef, useDebugValue, Children } from 'react';
import require$$1, { unstable_batchedUpdates, render, unmountComponentAtNode, createPortal } from 'react-dom';
import { Dropdown, IconEdit, IconUnlock, IconLock, Divider, IconTrash, Button, IconChevronDown, Modal, Typography, IconClipboard, IconKey, IconLink, Loading as Loading$1, IconMaximize2, IconPlus, IconX, Input, Popover as Popover$1, IconFilter, Toggle, IconMenu, IconList, IconRefreshCw, IconDownload, IconArrowLeft, InputNumber, IconArrowRight, Menu as Menu$1, IconSearch, IconExternalLink, Space } from '@supabase/ui';
import DataGrid, { Row, useRowSelection } from '@supabase/react-data-grid';
import require$$0, { jsx } from 'react/jsx-runtime';
...
var MonacoContainer = /*#__PURE__*/memo$1(MonacoContainer$1);
...
var index$1 = /*#__PURE__*/memo$1(Editor);
...
var DndProvider = memo$1(function DndProvider(_ref) {
...
const p=Symbol(),a$1=Symbol(),f$1="undefined"==typeof window||/ServerSideRendering/.test(window.navigator&&window.navigator.userAgent)?useEffect:useLayoutEffect,l$1=scheduler.exports.unstable_runWithPriority?e=>scheduler.exports.unstable_runWithPriority(scheduler.exports.unstable_NormalPriority,e):e=>e();function w(n){const o=createContext({[p]:{v:{current:n},n:{current:-1},l:new Set,u:e=>e()}});var c;return o[a$1]=o.Provider,o.Provider=(c=o.Provider,({value:e,children:n})=>{const o=useRef(e),u=useRef(0),s=useRef();if(!s.current){const e=new Set,r=r=>{unstable_batchedUpdates(()=>{u.current+=1,e.forEach(e=>e([u.current])),r();});};s.current={[p]:{v:o,n:u,l:e,u:r}};}return f$1(()=>{o.current=e,u.current+=1,l$1(()=>{s.current[p].l.forEach(r=>{r([u.current,e]);});});},[e]),createElement$1(c,{value:s.current},n)}),delete o.Consumer,o}function E(e,r){const t=useContext(e)[p];if("object"==typeof process&&"production"!==process.env.NODE_ENV&&!t)throw new Error("useContextSelector requires special context");const{v:{current:c},n:{current:u},l:s}=t,i=r(c),[d,a]=useReducer((e,t)=>{if(!t)return [c,i];if(t[0]<=u)return Object.is(e[1],i)?e:[c,i];try{if(2===t.length){if(Object.is(e[0],t[1]))return e;const n=r(t[1]);return Object.is(e[1],n)?e:[t[1],n]}}catch(e){}return [...e]},[c,i]);return Object.is(d[1],i)||a(),f$1(()=>(s.add(a),()=>{s.delete(a);}),[s]),d[1]}function b(e){const r=useContext(e)[p];if("object"==typeof process&&"production"!==process.env.NODE_ENV&&!r)throw new Error("useContextUpdate requires special context");const{u:t}=r;return t}
...
const createContainer = (useValue, concurrentMode = false) => {
  ...
  const UpdateContext = createContext(warningObject);

  const Provider = props => {
    const [state, update] = useValue(props);
    return createElement$1(UpdateContext.Provider, {
      value: update
    }, createElement$1(StateContext.Provider, {
      value: state
    }, props.children));
  };
  ...
  const useUpdate$1 = concurrentMode ? () => useUpdate(StateContext, UpdateContext) : () => useContext(UpdateContext);
...
function memo(Component, propsAreEqual) {
  const WrappedComponent = forwardRef((props, ref) => {
    ...
    return createElement$1(Component, _extends$1({}, props, {
      ref
    }));
  });
...
const ColumnMenu = ({ column }) => {
    ...
    function renderMenu() {
        return (React.createElement(React.Fragment, null,
            state.editable && onEditColumn !== undefined && (React.createElement(Dropdown.Item, { onClick: onEditColumn, icon: React.createElement(IconEdit, { size: "tiny" }) }, "Edit column")),
            React.createElement(Dropdown.Item, { onClick: column.frozen ? onUnfreezeColumn : onFreezeColumn, icon: column.frozen ? (React.createElement(IconUnlock, { size: "tiny" })) : (React.createElement(IconLock, { size: "tiny" })) }, column.frozen ? 'Unfreeze column' : 'Freeze column'),
            state.editable && onDeleteColumn !== undefined && (React.createElement(React.Fragment, null,
                React.createElement(Divider, { light: true }),
                React.createElement(Dropdown.Item, { onClick: onDeleteColumn, icon: React.createElement(IconTrash, { size: "tiny", stroke: "red" }) }, "Delete Column")))));
    }
    return (React.createElement(React.Fragment, null,
        React.createElement(Dropdown, { align: "end", side: "bottom", overlay: renderMenu() },
            React.createElement(Button, { as: 'span', type: "text", icon: React.createElement(IconChevronDown, null), style: { padding: '3px' } }))));
};
...
const BlockKeys = ({ onEscape, onEnter, value, children, }) => {
    ...
    return (React.createElement("div", { onKeyDown: handleKeyDown, onBlur: onBlur }, children));
};
...
const ConfirmModal = ({ title, message, onConfirm, onAsyncConfirm, variant = 'danger', }) => {
    ...
    return (React.createElement(Modal, { variant: variant, visible: true, title: title, description: message, showIcon: false, size: "small", confirmText: "OK", cancelText: "Cancel", onCancel: onCancelClick, onConfirm: onConfirmClick, loading: loading }));
};
...
function createElement(props) {
    ...
    if (divTarget) {
        render(React.createElement(ConfirmModal, Object.assign({}, props)), divTarget);
    }
    else {
...
const DropdownControl = (p) => {
    ...
    return (React.createElement(Dropdown, { className: className, side: side, align: align, overlay: React.createElement(DropdownItems, Object.assign({}, p)), isNested: isNested }, children));
};
const DropdownItems = ({ options, onSelect, }) => {
    return (React.createElement("div", { className: "dropdown-control", style: { maxHeight: '30vh' } },
        options.length == 0 && (React.createElement(Typography.Text, { className: "dropdown-control__empty-text" }, "No more items")),
        options.map((x) => {
            return (React.createElement(Dropdown.Item, { key: x.value, onClick: () => onSelect(x.value) }, x.label));
        })));
};
...
const MonacoEditor = ({ width, height, value, language, onChange, onMount, }) => {
    ...
    return (React.createElement(index$1, { width: width, height: height || '200px', theme: "supabase", wrapperProps: {
...
const NullValue = ({}) => {
    return (React.createElement("span", { className: "null-value", style: { opacity: 0.5 } }, "NULL"));
};
...
function useKeyboardShortcuts(keyMap, whitelistNodes = [], whitelistClasses = []) {
    const [lastKeydown, setLastKeydown] = React.useState();
...
const EmptyValue = ({}) => {
    return (React.createElement("span", { className: "sb-grid-empty-value", style: { opacity: 0.5 } }, "EMPTY"));
};
...
const RowMenu = ({}) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Dropdown, { align: "end", side: "bottom", overlay: React.createElement(React.Fragment, null,
                selectedRows.size == 0 && (React.createElement(Dropdown.Item, { onClick: onAllRowsExportCsv, icon: React.createElement(IconEdit, { size: "tiny" }) }, "Export all rows to csv")),
                selectedRows.size > 0 && (React.createElement(Dropdown.Item, { onClick: onRowsExportCsv, icon: React.createElement(IconEdit, { size: "tiny" }) }, "Export to csv")),
                editable && selectedRows.size > 0 && (React.createElement(React.Fragment, null,
                    React.createElement(Divider, { light: true }),
                    React.createElement(Dropdown.Item, { onClick: onRowsDelete, icon: React.createElement(IconTrash, { size: "tiny", stroke: "red" }) }, "Delete selected rows")))) },
            React.createElement(Button, { as: 'span', type: "text", icon: React.createElement(IconChevronDown, null), style: { padding: '3px' } }))));
};
...
const RowContextMenu = ({}) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Menu, { id: ROW_CONTEXT_MENU_ID, animation: false },
            React.createElement(Item, { onClick: onCopyCellContent },
                React.createElement(IconClipboard, { size: "tiny" }),
                React.createElement("span", { className: "sb-grid-context-menu__label" }, "Copy cell content")),
            React.createElement(Item, { onClick: onEditRowClick, hidden: isItemHidden, data: "edit" },
                React.createElement(IconEdit, { size: "tiny" }),
                React.createElement("span", { className: "sb-grid-context-menu__label" }, "Edit row")),
            React.createElement(Item, { onClick: onDeleteRow, hidden: isItemHidden, data: "delete" },
                React.createElement(IconTrash, { size: "tiny", stroke: "red" }),
                React.createElement("span", { className: "sb-grid-context-menu__label" }, "Delete row")))));
};
...
function ColumnHeader({ column, columnType, isPrimaryKey, format, }) {
    ...
    drag(drop(ref));
    return (React.createElement("div", { ref: ref, "data-handler-id": handlerId, style: { opacity } },
        React.createElement("div", { className: `sb-grid-column-header ${cursor}` },
            React.createElement("div", { className: "sb-grid-column-header__inner" },
                renderColumnIcon(columnType),
                isPrimaryKey && (React.createElement("div", { className: "sb-grid-column-header__inner__primary-key" },
                    React.createElement(IconKey, { size: "tiny", strokeWidth: 2 }))),
                React.createElement("span", { className: "sb-grid-column-header__inner__name" }, column.name),
                React.createElement("span", { className: "sb-grid-column-header__inner__format" }, columnFormat)),
            React.createElement(ColumnMenu, { column: column }))));
}
function renderColumnIcon(type) {
    switch (type) {
        case 'foreign_key':
            return React.createElement(IconLink, { size: "tiny", strokeWidth: 2 });
        default:
            return null;
    }
}
...
function RowRenderer(props) {
    ...
    return React.createElement(Row, Object.assign({}, props, { onContextMenu: displayMenu }));
}

function rowKeyGetter(row) {
    return row.idx;
}
const Grid = memo(React.forwardRef(({ width, height, containerClass, gridClass, rowClass }, ref) => {
    const dispatch = useDispatch();
    const state = useTrackedState();
    ...
    if (!columnHeaders || columnHeaders.length == 0) {
        return (React.createElement("div", { className: "sb-grid-grid--loading", style: { width: width || '100%', height: height || '50vh' } },
            React.createElement("div", { className: "sb-grid-grid--loading__inner" },
                React.createElement(Loading$1, { active: true },
                    React.createElement("div", null)),
                React.createElement(Typography.Text, { className: "sb-grid-grid--loading__inner__text" }, "loading ..."))));
    }
    return (React.createElement("div", { className: containerClass, style: { width: width || '100%', height: height || '50vh' } },
        React.createElement(DataGrid, { ref: ref, columns: gridColumns, rows: rows !== null && rows !== void 0 ? rows : [], rowRenderer: RowRenderer, rowKeyGetter: rowKeyGetter, selectedRows: state.selectedRows, onColumnResize: onColumnResize, onRowsChange: onRowsChange, onSelectedCellChange: onSelectedCellChange, onSelectedRowsChange: onSelectedRowsChange, className: gridClass, rowClass: rowClass, style: { height: '100%' } })));
}));
...
const SelectColumn = {
    ...
    headerRenderer: props => {
        return (React.createElement(SelectCellHeader, { "aria-label": "Select All", value: props.allRowsSelected, onChange: props.onAllRowsSelectionChange }));
    },
    formatter: (props) => {
        const [isRowSelected, onRowSelectionChange] = useRowSelection();
        return (React.createElement(SelectCellFormatter, { "aria-label": "Select", tabIndex: -1, isCellSelected: props.isCellSelected, value: isRowSelected, row: props.row, onChange: (checked, isShiftClick) => {
    ...
    groupFormatter: (props) => {
        const [isRowSelected, onRowSelectionChange] = useRowSelection();
        return (React.createElement(SelectCellFormatter, { "aria-label": "Select Group", tabIndex: -1, isCellSelected: props.isCellSelected, value: isRowSelected, onChange: checked => {
...
function SelectCellFormatter({ row, value, tabIndex, isCellSelected, disabled, onClick, onChange, 'aria-label': ariaLabel, 'aria-labelledby': ariaLabelledBy, }) {
    ...
    return (React.createElement("div", { className: "sb-grid-select-cell__formatter" },
        React.createElement("input", { "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, tabIndex: tabIndex, ref: inputRef, type: "checkbox", className: "rdg-row__select-column__select-action", disabled: disabled, checked: value, onChange: handleChange, onClick: onClick }),
        onEditRow && row && (React.createElement(Button, { type: "text", size: "tiny", className: "rdg-row__select-column__edit-action", icon: React.createElement(IconMaximize2, { size: "tiny" }), onClick: onEditClick, style: { padding: '2px' } }))));
}
function SelectCellHeader({ disabled, tabIndex, value, onChange, onClick, 'aria-label': ariaLabel, 'aria-labelledby': ariaLabelledBy, }) {
    function handleChange(e) {
        onChange(e.target.checked, e.nativeEvent.shiftKey);
    }
    return (React.createElement("div", { className: "sb-grid-select-cell__header" },
        React.createElement("input", { "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, tabIndex: tabIndex, type: "checkbox", className: "sb-grid-select-cell__header__input", disabled: disabled, checked: value, onChange: handleChange, onClick: onClick }),
        React.createElement(RowMenu, null)));
}
...
const AddColumn = {
    ...
    frozen: false,
    headerRenderer(props) {
        return (React.createElement(AddColumnHeader, { "aria-label": "Add New Row", value: props.allRowsSelected, onChange: props.onAllRowsSelectionChange }));
    },
};
const AddColumnHeader = () => {
    const state = useTrackedState();
    const { onAddColumn } = state;
    return (React.createElement("div", { className: "sb-grid-add-column" },
        React.createElement(Button, { block: true, type: "text", onClick: onAddColumn, icon: React.createElement(IconPlus, null) })));
};
...
const FilterRow = ({ filterIdx, now }) => {
    ...
    return (React.createElement(Dropdown.Misc, null,
        React.createElement("div", { className: "sb-grid-filter-row" },
            React.createElement("div", { className: "sb-grid-filter-row__inner" },
                React.createElement(Button, { icon: React.createElement(IconX, null), className: "sb-grid-filter-row__inner__close", shadow: false, size: "tiny", type: "text", onClick: onRemoveFilter }),
                React.createElement(DropdownControl, { align: "start", options: columnOptions, onSelect: onColumnChange },
                    React.createElement(Button, { as: "span", type: "outline", iconRight: React.createElement(IconChevronDown, null) }, (column === null || column === void 0 ? void 0 : column.name) || '')),
                React.createElement(DropdownControl, { align: "start", options: FilterOperatorOptions, onSelect: onOperatorChange },
                    React.createElement(Button, { as: "span", type: "outline", iconRight: React.createElement(IconChevronDown, null) }, filter.operator))),
            React.createElement("div", null,
                React.createElement(Input, { size: "tiny", value: filterValue, onChange: onFilterChange })))));
};
var FilterRow$1 = React.memo(FilterRow);
...
const FilterPopover = () => {
    ...
    const btnText = state.filters.length > 0
        ...
        : 'Filter';
    return (React__default.createElement(Popover$1, { align: "start", className: "sb-grid-filter-popover", overlay: React__default.createElement(Filter, null) },
        React__default.createElement(Button, { as: 'span', type: "text", icon: React__default.createElement(IconFilter, null), style: { padding: '4px 8px' } }, btnText)));
};
const Filter = () => {
    const state = useTrackedState();
    const dispatch = useDispatch();
    ...
    return (React__default.createElement("div", { className: "" },
        React__default.createElement("div", null,
            state.filters.map((_, index) => (React__default.createElement(FilterRow$1, { key: `filter-${index}`, filterIdx: index, now: Date.now() }))),
            state.filters.length == 0 && (React__default.createElement(Dropdown.Misc, null,
                React__default.createElement("div", { className: "sb-grid-filter-popover__misc" },
                    React__default.createElement(Typography.Text, null, "No filters applied to this view"),
                    React__default.createElement(Typography.Text, { small: true, type: "secondary", className: "sb-grid-filter-popover__misc__text" }, "Add a column below to filter the view"))))),
        React__default.createElement(Divider, { light: true }),
        React__default.createElement(Dropdown.Misc, null,
            React__default.createElement(Button, { icon: React__default.createElement(IconPlus, null), type: "text", onClick: onAddFilter }, "Add filter"))));
};
...
const SortRow = ({ columnName, index }) => {
    ...
    drag(drop(ref));
    return (React.createElement(Dropdown.Misc, null,
        React.createElement("div", { className: "sb-grid-sort-row", ref: ref, style: { opacity }, "data-handler-id": handlerId },
            React.createElement("div", { className: "sb-grid-sort-row__item" },
                React.createElement(Button, { icon: React.createElement(IconX, null), className: "sb-grid-sort-row__item__remove", shadow: false, size: "tiny", type: "text", onClick: onDeleteClick }),
                React.createElement("div", { className: "sb-grid-sort-row__item__label" },
                    React.createElement(Typography.Text, { type: "secondary", small: true }, index > 0 ? 'then by' : 'sort by'),
                    React.createElement(Typography.Text, null, column.name))),
            React.createElement("div", { className: "sb-grid-sort-row__item" },
                React.createElement(Toggle, { className: "sb-grid-sort-row__item_toogle", layout: "horizontal", label: "ascending", defaultChecked: sort.ascending, onChange: onToogle }),
                React.createElement(Typography.Text, { className: "sb-grid-sort-row__item__move" },
                    React.createElement(IconMenu, { size: "tiny" }))))));
};
var SortRow$1 = React.memo(SortRow);
...
const SortPopover = () => {
    ...
    const btnText = state.sorts.length > 0
        ...
        : 'Sort';
    return (React__default.createElement(Popover$1, { align: "start", className: "sb-grid-sort-popover", overlay: React__default.createElement(Sort, null) },
        React__default.createElement(Button, { as: 'span', type: "text", icon: React__default.createElement(IconList, null), style: { padding: '4px 8px' } }, btnText)));
};
const Sort = () => {
    var _a;
    const state = useTrackedState();
    ...
    return (React__default.createElement("div", null,
        state.sorts.map((x, index) => (React__default.createElement(SortRow$1, { key: x.column, columnName: x.column, index: index }))),
        state.sorts.length == 0 && (React__default.createElement(Dropdown.Misc, null,
            React__default.createElement("div", { className: "sb-grid-dropdown__empty" },
                React__default.createElement(Typography.Text, null, "No sorts applied to this view"),
                React__default.createElement(Typography.Text, { small: true, type: "secondary", className: "sb-grid-dropdown__empty__text" }, "Add a column below to sort the view")))),
        React__default.createElement(Divider, { light: true }),
        React__default.createElement(Dropdown.Misc, null, columns && columns.length > 0 ? (React__default.createElement(DropdownControl, { options: dropdownOptions, onSelect: onAddSort, side: "bottom", align: "start" },
            React__default.createElement(Button, { as: "span", type: "text", iconRight: React__default.createElement(IconChevronDown, null), className: "sb-grid-dropdown__item-trigger" }, `Pick ${state.sorts.length > 1 ? 'another' : 'a'} column to sort by`))) : (React__default.createElement(Typography.Text, { small: true, type: "secondary" }, "All columns have been added")))));
};
...
const StatusLabel = ({}) => {
    ...
    return (React.createElement("div", { className: "sb-grid-status-label" },
        msg && React.createElement(Typography.Text, null, msg),
        !msg && (React.createElement("div", { className: "sb-grid-status-label__no-msg" },
            React.createElement("div", null)))));
};
...
const RefreshButton = ({}) => {
    ...
    return (React.createElement(Button, { type: "text", style: { padding: '4px 8px' }, icon: React.createElement(IconRefreshCw, null), onClick: onClick, loading: loading }, "Refresh"));
};

const Header = ({ onAddColumn, onAddRow, headerActions, }) => {
    const state = useTrackedState();
    const { selectedRows } = state;
    return (React.createElement("div", { className: "sb-grid-header" },
        React.createElement("div", { className: "sb-grid-header__inner" }, selectedRows.size > 0 ? (React.createElement(RowHeader, null)) : (React.createElement(DefaultHeader, { onAddColumn: onAddColumn, onAddRow: onAddRow }))),
        React.createElement("div", { className: "sb-grid-header__inner" },
            headerActions,
...
const DefaultHeader = ({ onAddColumn, onAddRow, }) => {
    const renderNewColumn = (onAddColumn) => {
        if (!onAddColumn)
            return null;
        return (React.createElement(Button, { type: "text", onClick: onAddColumn, style: { padding: '4px 8px' } }, "New Column"));
    };
    const renderAddRow = (onAddRow) => {
        if (!onAddRow)
            return null;
        return (React.createElement(Button, { style: { padding: '4px 8px' }, icon: React.createElement(IconPlus, { size: "tiny" }), onClick: onAddRow }, "Insert row"));
    };
    return (React.createElement(React.Fragment, null,
        React.createElement(RefreshButton, null),
        React.createElement(FilterPopover, null),
        React.createElement(SortPopover, null),
        React.createElement(Divider, { light: true, type: "vertical", className: "sb-grid-header__inner__divider" }),
...
const RowHeader = ({}) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Button, { type: "text", style: { padding: '4px' }, icon: React.createElement(IconX, { size: "tiny", strokeWidth: 2 }), onClick: deselectRows }),
        React.createElement("div", null,
            React.createElement(Typography.Text, { small: true, type: "secondary", className: "row_header__selected-rows" }, selectedRows.size > 1
                ? `${selectedRows.size} rows selected`
                : `${selectedRows.size} row selected`)),
        React.createElement(Button, { type: "primary", size: "tiny", style: { padding: '4px 8px' }, icon: React.createElement(IconDownload, null), onClick: onRowsExportCsv }, "Export to csv"),
        editable && (React.createElement(React.Fragment, null,
            React.createElement(Divider, { type: "vertical", className: "sb-grid-header__inner__divider", light: true }),
            React.createElement(Button, { type: "default", size: "tiny", style: { padding: '4px 8px' }, icon: React.createElement(IconTrash, { size: "tiny" }), onClick: onRowsDelete }, selectedRows.size > 1
...
const Pagination = () => {
    ...
    return (React.createElement("div", { className: "sb-grid-pagination" }, state.totalRows < 0 ? (React.createElement(Typography.Text, null, "... loading total records")) : (React.createElement(React.Fragment, null,
        React.createElement(Button, { icon: React.createElement(IconArrowLeft, null), type: "outline", disabled: state.page <= 1, onClick: onPreviousPage, style: { padding: '3px 10px' } }),
        React.createElement(Typography.Text, null, "Page"),
        React.createElement("div", { className: "sb-grid-pagination-input-container" },
            React.createElement(InputNumber, { value: page, onChange: onPageChange, className: "sb-grid-pagination-input", size: "tiny", style: {
                    width: '3rem',
                }, max: maxPages, min: 1 })),
        React.createElement(Typography.Text, null, `of ${totalPages}`),
        React.createElement(Button, { icon: React.createElement(IconArrowRight, null), type: "outline", disabled: state.page >= maxPages, onClick: onNextPage, style: { padding: '3px 10px' } }),
        React.createElement(DropdownControl, { options: rowsPerPageOptions, onSelect: onRowsPerPageChange, side: "top", align: "start" },
            React.createElement(Button, { as: "span", type: "outline", style: { padding: '3px 10px' } }, `${state.rowsPerPage} rows`)),
        React.createElement(Typography.Text, null, `${state.totalRows.toLocaleString()} records`)))));
};

const Footer = () => {
    return (React.createElement("div", { className: "sb-grid-footer" },
        React.createElement("div", { className: "sb-grid-footer__inner" },
...
function BooleanEditor({ row, column, onRowChange, onClose, }) {
    ...
    return (React.createElement("div", { className: "sb-grid-checkbox-editor" },
        React.createElement("input", { className: "sb-grid-checkbox-editor__input", checked: row[column.key], onChange: onChange, onBlur: onBlur, type: "checkbox", style: { margin: 'auto' } })));
}
...
function DateEditor({ row, column, onRowChange, onClose, }) {
    ...
    return (React.createElement("input", { className: "sb-grid-date-editor", ref: autoFocusAndSelect$3, value: value !== null && value !== void 0 ? value : '', onChange: onChange, onBlur: () => onClose(true), type: "date" }));
}
...
function BaseEditor$1({ row, column, format, onRowChange, onClose, }) {
    ...
    return (React.createElement("input", { className: "sb-grid-datetime-editor", ref: autoFocusAndSelect$2, value: timeValue !== null && timeValue !== void 0 ? timeValue : '', onChange: onChange, onBlur: () => onClose(true), type: "datetime-local", step: "1" }));
}
function DateTimeEditor(props) {
    return React.createElement(BaseEditor$1, Object.assign({}, props, { format: "YYYY-MM-DDTHH:mm:ss" }));
}
function DateTimeWithTimezoneEditor(props) {
    return React.createElement(BaseEditor$1, Object.assign({}, props, { format: "YYYY-MM-DDTHH:mm:ssZ" }));
}
...
function JsonEditor({ row, column, onRowChange, }) {
    ...
    const [value, setValue] = React.useState(prettyJsonValue);
    ...
    return (React.createElement(Popover.Popover, { isOpen: isPopoverOpen, padding: -35, containerClassName: "", positions: ['bottom', 'top', 'left'], align: "start", content: React.createElement(BlockKeys, { value: value, onEscape: onEscape },
            React.createElement(MonacoEditor, { width: `${(gridColumn === null || gridColumn === void 0 ? void 0 : gridColumn.width) || column.width}px`, value: value !== null && value !== void 0 ? value : '', language: "json", onChange: onChange })) },
        React.createElement("div", { className: `${!!value && jsonString.trim().length == 0
                ? 'sb-grid-fill-container'
                : ''} sb-grid-json-editor__trigger`, onClick: () => setIsPopoverOpen(!isPopoverOpen) }, value === null || value === '' ? React.createElement(NullValue, null) : jsonString)));
}
...
function NullableBooleanEditor({ row, column, onRowChange, onClose, }) {
    ...
    return (React__default.createElement("div", { className: "sb-grid-checkbox-editor" },
        React__default.createElement("input", { className: "sb-grid-checkbox-editor__input", checked: value === null ? false : value, onChange: onChange, onBlur: onBlur, type: "checkbox", style: { margin: 'auto' } }),
        React__default.createElement(Button, { type: "text", title: "Clear", icon: React__default.createElement(IconX, { size: "tiny", strokeWidth: 2 }), onClick: onClear, style: { padding: '3px', margin: 'auto 5px auto auto' } })));
}
...
function NumberEditor({ row, column, onRowChange, onClose, }) {
    ...
    return (React.createElement("input", { className: "sb-grid-number-editor", ref: autoFocusAndSelect$1, value: value !== null && value !== void 0 ? value : '', onChange: onChange, onBlur: onBlur, type: "number" }));
}
...
function SelectEditor({ row, column, onRowChange, onClose, options, }) {
    ...
    return (React.createElement("select", { className: "sb-grid-select-editor", value: value !== null && value !== void 0 ? value : '', onChange: onChange, onBlur: onBlur, autoFocus: true },
        React.createElement("option", { value: '' }, "[null]"),
        options.map(({ label, value }) => (React.createElement("option", { key: value, value: value }, label)))));
}
...
function TextEditor({ row, column, onRowChange, }) {
    ...
    return (React.createElement(Popover.Popover, { isOpen: isPopoverOpen, padding: -35, containerClassName: "", positions: ['bottom', 'top', 'left'], align: "start", content: React.createElement(BlockKeys, { value: value, onEscape: onEscape },
            React.createElement(MonacoEditor, { width: `${(gridColumn === null || gridColumn === void 0 ? void 0 : gridColumn.width) || column.width}px`, value: value !== null && value !== void 0 ? value : '', onChange: onChange })) },
        React.createElement("div", { className: `${!!value && value.trim().length == 0 ? 'sb-grid-fill-container' : ''} sb-grid-text-editor__trigger`, onClick: () => setIsPopoverOpen(!isPopoverOpen) }, value === null ? React.createElement(NullValue, null) : value === '' ? React.createElement(EmptyValue, null) : value)));
}
...
function BaseEditor({ row, column, format, onRowChange, onClose, }) {
    ...
    return (React.createElement("input", { className: "sb-grid-time-editor", ref: autoFocusAndSelect, value: timeValue !== null && timeValue !== void 0 ? timeValue : '', onChange: onChange, onBlur: () => onClose(true), type: "time", step: "1" }));
}
function TimeEditor(props) {
    return React.createElement(BaseEditor, Object.assign({}, props, { format: "HH:mm:ss" }));
}
function TimeWithTimezoneEditor(props) {
    return React.createElement(BaseEditor, Object.assign({}, props, { format: "HH:mm:ssZZ" }));
}
...
const BooleanFormatter = (p) => {
    ...
    return React.createElement(React.Fragment, null, value ? 'true' : 'false');
};
...
const DefaultFormatter = (p) => {
    ...
    if (value === null)
        return React.createElement(NullValue, null);
    if (value === '')
        return React.createElement(EmptyValue, null);
    if (typeof value == 'object' || Array.isArray(value)) {
...
const RowItem = ({ item, onSelect }) => {
    ...
    return (React.createElement("div", { className: "foreign-table-modal__row-item" },
        React.createElement(Menu$1.Item, { onClick: () => onSelect(item) },
            React.createElement("div", { className: "foreign-table-modal__row-item__inner" }, keys.map((key, j) => {
                //
                ...
                if (j > 5)
                    return null;
                return (React.createElement("div", { className: "foreign-table-modal__row-item__inner__key-item", key: `item-${j}` },
                    React.createElement(Typography.Text, { small: true, type: "secondary", className: "foreign-table-modal__row-item__inner__key-item__key" }, key),
                    React.createElement(Typography.Text, { small: true, strong: true }, item[key] || '[null]')));
            })))));
};
...
const FilterHeader = ({ defaultColumnName, defaultValue, foreignColumnNames, onChange, }) => {
    ...
    const [filterText, setFilterText] = useState(defaultValue !== null && defaultValue !== void 0 ? defaultValue : '');
    ...
    return (React__default.createElement("div", { className: "foreign-table-modal__filter" },
        React__default.createElement(DropdownControl, { side: "bottom", align: "start", options: columnOptions, onSelect: onColumnChange },
            React__default.createElement(Button, { as: "span", type: "outline", iconRight: React__default.createElement(IconChevronDown, null) },
                React__default.createElement("span", { className: "foreign-table-modal__filter__trigger-content" },
                    React__default.createElement("span", { className: "foreign-table-modal__filter__trigger-content__label" }, "Column"),
                    React__default.createElement("span", { className: "foreign-table-modal__filter__trigger-content__name" }, columnName)))),
        React__default.createElement(DropdownControl, { side: "bottom", align: "start", options: FilterOperatorOptions, onSelect: onConditionChange },
            React__default.createElement(Button, { as: "span", type: "outline", iconRight: React__default.createElement(IconChevronDown, null) },
                React__default.createElement("span", { className: "foreign-table-modal__filter__trigger-content" },
                    React__default.createElement("span", { className: "foreign-table-modal__filter__trigger-content__label" }, "Filter"),
                    React__default.createElement("span", { className: "foreign-table-modal__filter__trigger-content__name" }, condition)))),
        React__default.createElement(Input, { size: "tiny", className: "foreign-table-modal__filter__search-input", placeholder: "Find a record", value: filterText, onChange: onFilterChange, icon: React__default.createElement(IconSearch, { size: "small" }) })));
};
...
const ForeignTableModal = ({ columnName, defaultValue, onChange, }) => {
    ...
    const [foreignColumnNames, setForeignColumnNames] = React.useState([]);
    ...
    function renderRows() {
        ...
        const temp = rows.map((x, i) => {
            return React.createElement(RowItem, { key: `menu-${i}`, item: x, onSelect: onItemSelect });
        });
        return React.createElement(Menu$1, { className: "foreign-table-modal__menu" }, temp);
    }
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Button, { type: "text", onClick: toggle, icon: React.createElement(IconExternalLink, null), style: { padding: '3px' } }),
        React.createElement(Modal, { visible: visible, onCancel: toggle, closable: true, contentStyle: { padding: 0 }, customFooter: React.createElement(Space, { style: { width: '100%' } },
                React.createElement(Button, { block: true, danger: true, icon: React.createElement(IconTrash, null), onClick: onClearValueClick }, "Clear value")) },
            React.createElement(FilterHeader, { defaultColumnName: (_b = columnDefinition === null || columnDefinition === void 0 ? void 0 : columnDefinition.targetColumnName) !== null && _b !== void 0 ? _b : undefined, defaultValue: defaultValue, foreignColumnNames: foreignColumnNames, onChange: onFilterChange }),
            React.createElement("div", { className: "foreign-table-modal__content-container" },
                React.createElement(Divider, { light: true }),
                React.createElement("div", { className: "foreign-table-modal__content-container__inner" },
                    React.createElement("div", { className: "foreign-table-modal__content-container__inner__overflow", style: { minHeight: '21rem', maxHeight: '20rem' } }, renderRows()))))));
};
...
const ForeignKeyFormatter = (p) => {
    ...
    return (React.createElement("div", { className: "sb-grid-foreign-key-formatter" },
        React.createElement("p", { className: "sb-grid-foreign-key-formatter__text" }, value === null ? React.createElement(NullValue, null) : value),
        React.createElement(ForeignTableModal, { columnName: p.column.key, defaultValue: value, onChange: onChange })));
};
...
function getGridColumns(table, options) {
    const columns = table.columns.map((x) => {
        ...
        const columnDefinition = {
            ...
            headerRenderer: (props) => (React.createElement(ColumnHeader, Object.assign({}, props, { columnType: columnType, isPrimaryKey: x.isPrimaryKey, format: x.format }))),
...
function getColumnEditor(columnDefinition, columnType) {
    ...
    switch (columnType) {
        ...
        case 'enum': {
            ...
            return (p) => React.createElement(SelectEditor, Object.assign({}, p, { options: options }));
        }
        case 'array':
        case 'json': {
            return JsonEditor;
 ...
 */
const SupabaseGrid = React__default.forwardRef((props, ref) => {
    ...
    return (React__default.createElement(StoreProvider, null,
...
const SupabaseGridLayout = React__default.forwardRef((props, ref) => {
    ...
    const [mounted, setMount] = React__default.useState(false);
    ...
    return (React__default.createElement("div", { className: "sb-grid" },
        React__default.createElement(Header, { onAddRow: editable ? props.onAddRow : undefined, onAddColumn: editable ? props.onAddColumn : undefined, headerActions: headerActions }),
        React__default.createElement(Grid, Object.assign({ ref: gridRef }, gridProps)),
        React__default.createElement(Footer, null),
        React__default.createElement(Shortcuts, { gridRef: gridRef }),
